...
mha_bwd(const at::Tensor &dout,                   // batch_size x seqlen_q x num_heads, x multiple_of(head_size_v, 8)
        const at::Tensor &q,                      // batch_size x seqlen_q x num_heads x head_size
        const at::Tensor &k,                      // batch_size x seqlen_k x num_heads_k x head_size
        const at::Tensor &v,                      // batch_size x seqlen_k x num_heads_k x head_size_v
        const at::Tensor &out,                    // batch_size x seqlen_q x num_heads x head_size_v
        const at::Tensor &softmax_lse,            // b x h x seqlen_q
        std::optional<at::Tensor> &dq_,           // batch_size x seqlen_q x num_heads x head_size
        std::optional<at::Tensor> &dk_,           // batch_size x seqlen_k x num_heads_k x head_size
        std::optional<at::Tensor> &dv_,           // batch_size x seqlen_k x num_heads_v x head_size_v
        ...
        std::optional<at::Tensor> &rng_state_)
{
    ...
    const ck_tile::index_t head_size_max = std::max(head_size, head_size_v);
    if (!deterministic) {
        dq_accum = torch::zeros({1, batch_size, seqlen_q, num_heads, head_size_max}, opts.dtype(at::kFloat));
    } else {
        const ck_tile::index_t kN0 = head_size_max <= 128 ? 128 : 64;
        const ck_tile::index_t nsplits = ck_tile::integer_divide_ceil(seqlen_k, kN0);
        dq_accum = torch::zeros({nsplits, batch_size, seqlen_q, num_heads, head_size_max}, opts.dtype(at::kFloat));
    }
    ...
    if (seqlen_q > 0) {
        ...
        auto traits =
            get_ck_fmha_bwd_traits(mask, q_dtype_str, head_size, head_size_v, is_dropout, alibi_slopes_.has_value(), deterministic);
