...
def test_flash_attn_output(
    seqlen_q, seqlen_k, d, v_d, dropout_p, causal, local, alibi, deterministic, mha_type, dtype, kvpacked
):
    if (
    ...
    batch_size = 4
    nheads = 6  # softcap reference impl takes more memory
    ...
    v = torch.randn(batch_size, seqlen_k, nheads_k, v_d, device=device, dtype=dtype, requires_grad=True)
    if alibi:
